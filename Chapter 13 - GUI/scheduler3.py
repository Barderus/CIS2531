...
class ScheduleGUI:
    ...
    def add_student_from_file(self, stu_id, stu_name, stu_gpa, stu_courses):
        ''' This method adds a student to the list from a file '''
        # Convert GPA to float
        stu_gpa = float(stu_gpa)

        # Create a Student object
        new_student = stu.Student(stu_id, stu_name, stu_gpa)


        for course_info in stu_courses:
            new_student.addCourse(course_info)
        
        self.__student_list.append(new_student)
        
        for student in self.__student_list:
            student_info = (f'ID: {student.id_num}, Name: {student.name}, GPA: {student.gpa}\n'
                        f'Courses: {", ".join([f"{course.name} ({course.crHours} credit hours)" for course in student.courses])}\n')

        self.stulistbox.insert(tkinter.END, student_info)

        # PRINTING FOR TESTING
        print("testing...")
        for items in self.__student_list:
            #print(items)
            print(items.courses)
            ...
            with open(file_path, "r") as file:
                for line in file:
                    ...
                    if len(fields) >= 3:
                        ...
                        # Extract the courses information
                        course_list = [course.strip() for course in last_field]
                        print(f"ID: {id_num}, Name: {name}, GPA: {gpa}, Last Field: {course_list}")
                        
                        self.add_student_from_file(id_num, name, gpa, course_list)
                        

                        ''' It prints ID: 12345, Name: Fiona Carruth, GPA: 4.0, Last Field: ['Intro to Biology (5 credit hours)'] '''
                        ...
                        return
                print(f"ID: {id_num}, Name: {name}, GPA: {gpa}, Last Field: {last_field}")
