...
def displayHand(player_hand, player_name, deal_num):
    if isinstance(player_hand, dict):
        player_value = sum(player_hand.values())
        hand_str = ""
        for card in player_hand.keys():
            hand_str += card + " "
        print(f"{deal_num:<5} {player_name:<10} {hand_str}")
        return player_value
    else:
...
def playRound(deck, player1_wins, player2_wins, rounds_played):
    ...
    # Deal initial two cards to both players
    player1_hand = dealCards(shuffledDeck, 2)
    player2_hand = dealCards(shuffledDeck, 2)

    # Keep dealing cards until one player's hand value exceeds 21
    while True:
        # Display the current round summary
        displayHand(player1_hand, "Player 1", rounds_played)
        displayHand(player2_hand, "Player 2", rounds_played)

        # Calculate player hand values
        player1_value = sum(player1_hand.values())
        player2_value = sum(player2_hand.values())

        # Check if either player's hand value exceeds 21
        if player1_value > 21 or player2_value > 21:
            break

        # Deal one more card to each player
        player1_hand.append(dealCards(shuffledDeck))
        player2_hand.append(dealCards(shuffledDeck))
...
def displayRound(player1_hand, player2_hand):
    print("--> ** NEW ROUND **")
    print("Deal   Player1          Player2")
    print("----   -------          -------")

    player1_value = displayHand(player1_hand, "Player 1", 1)
    player2_value = displayHand(player2_hand, "Player 2", 2)

    print("================================   ========")
    print(f"Hand value:   {player1_value:<10}   {player2_value:<10}")
